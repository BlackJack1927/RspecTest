...
RSpec.describe ExchangeIt::Account do
  let(:user_class) { Struct.new(:name, :surname) } # класс заглушка
  ...
  context 'when performing money withdrawal' do
    ...
    specify '#transfer_with_converstion' do
      allow(john).to receive(:convert).with(sum: 50, from: :usd, to: :eur).and_return(40)#частичная заглушка для обьекта джон метода конверт
    ...
    specify '#transfer' do
     ...
     #specify 'transfer (mocked)' do #tesr #transfer кторий віше с заглушкой
